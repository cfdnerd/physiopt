...
# Pin xformers to 0.0.27.post2 to match torch 2.4.0 and avoid upgrading to 2.5.1
pip install xformers==0.0.27.post2 --index-url https://download.pytorch.org/whl/cu121
MAX_JOBS=4 pip install flash-attn --no-build-isolation
pip install "jupyter_client<8.0.0"
pip install tbb
pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu121.html

# Clean up extensions directory to avoid 'already exists' errors
rm -rf /tmp/extensions
mkdir -p /tmp/extensions

git clone https://github.com/NVlabs/nvdiffrast.git /tmp/extensions/nvdiffrast
# Add --no-build-isolation to avoid build errors
